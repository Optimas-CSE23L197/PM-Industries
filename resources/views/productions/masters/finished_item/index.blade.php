                    ...
                    <table id="depttable" class="table table-sm table-bordered">
                        <thead class="thead-light">
                            <tr>
                                ...
                                <th style="width:15%; text-align: right">Standard Cost</th>
                                <th style="width:10%">Status</th>
                                <th style="width:10%">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse( $finishedItems as $f )
                                @php
                                    $status = $f['activeyn'] ?? 'N';
                                    $btnColor = $aedl = '';
                                    if( $status === 'Y' ) {
                                ...
                                <tr>
                                    <td>{{ $f['name'] ?? '' }}</td>
                                    <td>{{ $f['unit'] ?? '' }}</td>
                                    <td style="text-align: right;">{{ $f['standard_cost'] ?? '' }}</td>
                                    <td>
                                        <button type='button' class="btn btn-xs {{ $btnColor }} btn-block"
                                            onclick="mtd.show_msg(2, '{{ route('finishedItem.stat', ['code' => $f['code'], 'actyn' => $f['activeyn']]) }}', 'Are you sure to change its status..?', 2);"
                                        >
                                            {{ $status }}
                                        </button>
                                    </td>
                                    <td class="dropdown text-center">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-md dropdown-menu-right">
                                            <a href="{{ route('finishedItemDetails', ['vwedt' => 1, 'code' => $f['code']]) }}" class="dropdown-item">
                                                <div class="media">
                                            ...
                                            <a href="{{ route('finishedItemDetails', ['vwedt' => 2, 'code' => $f['code']]) }}" class="dropdown-item">
                                                <div class="media">
                                                    <div class="media-body">
                                                        <p>Edit</p>
                                                    </div>
