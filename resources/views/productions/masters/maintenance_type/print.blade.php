...
@section('content')
    <div class="card">
        <div class="card-header py-1">
            <label class="card-title">Maintenance Type</label>
        ...
        <div class="card-body print">
            ...
            <div style="text-align: center; margin-bottom: 20px; border-bottom: 2px solid #343a40; padding-bottom: 10px;">
                ...
                <h5 style="margin: 10px 0 0 0; text-transform: uppercase; letter-spacing: 2px; color: #343a40;">
                    Maintenance Type Master List
                </h5>
            </div>
            {{-- company header end --}}

            <table class="table table-bordered" width="100%" border="1"
                data-tableName="@php echo 'Maintenance-Type-Master_'.date('dmHis'); @endphp" data-pageO="p"
                ...
                <tbody>
                    @forelse($maintenanceTypes as $i => $mt)
                        <tr style="{{ $i % 2 == 0 ? 'background-color: #f8f9fa' : 'background-color: #fff' }}">
                            <td style="text-align: center">{{ $i + 1 }}</td>
                            <td>{{ $mt['name'] ?? '' }}</td>
                            <td style="color: {{ ($mt['activeyn'] ?? 'N') === 'Y' ? 'green' : 'red' }}; text-align: center;">
                                {{ ($mt['activeyn'] ?? 'N') === 'Y' ? 'Active' : 'Inactive' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
            ...
            <div style="margin-top: 20px; border-top: 2px solid #343a40; padding-top: 10px;">
                <div style="display: flex; justify-content: space-between;">
                    <div>
                        <strong>Total Records:</strong> {{ is_array($maintenanceTypes) ? count($maintenanceTypes) : 0 }}
                    </div>
                    <div>
