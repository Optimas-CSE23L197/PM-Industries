...
@section('content')
    <div class="card">
        <div class="card-header py-1">
            <label class="card-title">Enquiry</label>
        ...
        <div class="card-body print">
            ...
            <div style="text-align: center; margin-bottom: 20px; border-bottom: 2px solid #343a40; padding-bottom: 10px;">
                ...
                <h5 style="margin: 10px 0 0 0; text-transform: uppercase; letter-spacing: 2px; color: #343a40;">
                    Enquiry Transaction List
                </h5>
            </div>
            {{-- company header end --}}

            <table class="table table-bordered" width="100%" border="1"
                data-tableName="@php echo 'Enquiry-List_'.date('dmHis'); @endphp" data-pageO="l"
                data-pdfmode="download" cellspacing="0" cellpadding="4">
                <thead>
                    <tr style="background-color: #343a40; color: #fff;">
                        <th style="width: 6%; text-align: center;">Sl. No</th>
                        <th style="width: 18%; text-align: left;">Enquiry No.</th>
                        <th style="width: 25%; text-align: left;">Customer</th>
                        <th style="width: 20%; text-align: left;">Lead Source</th>
                        <th style="width: 16%; text-align: center;">Expected Order Date</th>
                        <th style="width: 15%; text-align: center;">Status</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($enquiries as $i => $e)
                        <tr style="{{ $i % 2 == 0 ? 'background-color: #f8f9fa' : 'background-color: #fff' }}">
                            <td style="text-align: center">{{ $i + 1 }}</td>
                            <td>{{ $e['enquiry_no'] ?? '' }}</td>
                            <td>{{ $e['customer_name'] ?? '' }}</td>
                            <td>{{ $e['leadsource_name'] ?? '' }}</td>
                            <td style="text-align: center">{{ $e['expected_order_date'] ?? '' }}</td>
                            <td style="color: {{ ($e['activeyn'] ?? 'N') === 'Y' ? 'green' : 'red' }}; text-align: center;">
                                {{ ($e['activeyn'] ?? 'N') === 'Y' ? 'Active' : 'Inactive' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
            ...
            <div style="margin-top: 20px; border-top: 2px solid #343a40; padding-top: 10px;">
                <div style="display: flex; justify-content: space-between;">
                    <div>
                        <strong>Total Records:</strong> {{ is_array($enquiries) ? count($enquiries) : 0 }}
                    </div>
                    <div>
