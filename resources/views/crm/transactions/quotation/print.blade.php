...
@section('content')
    <div class="card">
        <div class="card-header py-1">
            <label class="card-title">Quotation</label>
        ...
        <div class="card-body print">
            ...
            <table class="table table-bordered" width="100%" border="1"
                data-tableName="@php echo 'Quotation-List_'.date('dmHis'); @endphp" data-pageO="l"
                ...
                <thead>
                    <tr style="background-color: #343a40; color: #fff;">
                        ...
                        <th style="width: 16%; text-align: left;">Enquiry No.</th>
                        <th style="width: 20%; text-align: left;">Customer</th>
                        <th style="width: 10%; text-align: center;">Valid Till</th>
                ...
                <tbody>
                    @forelse($quotations as $i => $q)
                        <tr style="{{ $i % 2 == 0 ? 'background-color: #f8f9fa' : 'background-color: #fff' }}">
                            ...
                            <td style="text-align: center">{{ $q['quotation_date'] ?? '' }}</td>
                            <td>{{ $q['enquiry_no'] ?? '' }}</td>
                            <td>{{ $q['customer_name'] ?? '' }}</td>
                            <td style="text-align: center">{{ $q['valid_until'] ?? '' }}</td>
                            <td style="text-align: right">{{ number_format((float)($q['net_amount'] ?? 0), 2) }}</td>
                            <td style="color: {{ ($q['activeyn'] ?? 'N') === 'Y' ? 'green' : 'red' }}; text-align: center;">
                                {{ ($q['activeyn'] ?? 'N') === 'Y' ? 'Active' : 'Inactive' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
            ...
            <div style="margin-top: 20px; border-top: 2px solid #343a40; padding-top: 10px;">
                <div style="display: flex; justify-content: space-between;">
                    <div>
                        <strong>Total Records:</strong> {{ is_array($quotations) ? count($quotations) : 0 }}
                    </div>
                    <div>
