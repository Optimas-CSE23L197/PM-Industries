...
@section('content')
    <div class="card">
        <div class="card-header py-1">
            <label class="card-title">Sales Order</label>
        ...
        <div class="card-body print">
            ...
            <table class="table table-bordered" width="100%" border="1"
                data-tableName="@php echo 'Sales-Order_'.date('dmHis'); @endphp" data-pageO="l"
                ...
                <thead>
                    <tr style="background-color: #343a40; color: #fff;">
                        ...
                        <th style="width: 12%; text-align: center;">Order Date</th>
                        <th style="width: 20%; text-align: left;">Customer</th>
                        <th style="width: 16%; text-align: left;">Quot. No.</th>
                        <th style="width: 12%; text-align: center;">Delivery Date</th>
                        <th style="width: 10%; text-align: right;">Amount</th>
                        <th style="width: 8%;  text-align: center;">Status</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($salesOrders as $i => $so)
                        <tr style="{{ $i % 2 == 0 ? 'background-color: #f8f9fa' : 'background-color: #fff' }}">
                            <td style="text-align: center">{{ $i + 1 }}</td>
                            <td>{{ $so['order_no'] ?? '' }}</td>
                            <td style="text-align: center">{{ $so['order_date'] ?? '' }}</td>
                            <td>{{ $so['customer_name'] ?? '' }}</td>
                            <td>{{ $so['quotation_no'] ?? '' }}</td>
                            <td style="text-align: center">{{ $so['delivery_date'] ?? '' }}</td>
                            <td style="text-align: right">{{ number_format((float)($so['total_amount'] ?? 0), 2) }}</td>
                            <td style="color: {{ ($so['activeyn'] ?? 'N') === 'Y' ? 'green' : 'red' }}; text-align: center;">
                                {{ ($so['activeyn'] ?? 'N') === 'Y' ? 'Active' : 'Inactive' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
            ...
            <div style="margin-top: 20px; border-top: 2px solid #343a40; padding-top: 10px;">
                <div style="display: flex; justify-content: space-between;">
                    <div>
                        <strong>Total Records:</strong> {{ is_array($salesOrders) ? count($salesOrders) : 0 }}
                    </div>
                    <div>
