    ...
    <div class="card">
        ...
        <div class="card-body print">
            ...
            <table class="table table-bordered" width="100%" border="1"
                data-tableName="@php echo 'Lead-Source-Master_'.date('dmHis'); @endphp" data-pageO="p"
                ...
                <tbody>
                    @forelse($leadSources as $i => $ls)
                        <tr style="{{ $i % 2 == 0 ? 'background-color: #f8f9fa' : 'background-color: #fff' }}">
                            <td style="text-align: center">{{ $i + 1 }}</td>
                            <td>{{ $ls['name'] ?? '' }}</td>
                            <td style="color: {{ ($ls['activeyn'] ?? 'N') === 'Y' ? 'green' : 'red' }}; text-align: center;">
                                {{ ($ls['activeyn'] ?? 'N') === 'Y' ? 'Active' : 'Inactive' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
            ...
            <div style="margin-top: 20px; border-top: 2px solid #343a40; padding-top: 10px;">
                <div style="display: flex; justify-content: space-between;">
                    <div>
                        <strong>Total Records:</strong> {{ is_array($leadSources) ? count($leadSources) : 0 }}
                    </div>
                    <div>
