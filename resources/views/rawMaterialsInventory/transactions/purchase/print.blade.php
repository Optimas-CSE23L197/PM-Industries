...
@section('content')
    ...
    <div class="card">
        <div class="card-header py-1">
            <label class="card-title">Purchase</label>
        ...
        <div class="card-body print">
            ...
            <div style="text-align: center; margin-bottom: 20px; border-bottom: 2px solid #343a40; padding-bottom: 10px;">
                ...
                <h5 style="margin: 10px 0 0 0; text-transform: uppercase; letter-spacing: 2px; color: #343a40;">Purchase Transaction List</h5>
            </div>
            {{-- company header end --}}

            <table class="table table-bordered" width="100%" border="1"
                data-tableName="@php echo 'Purchase-Transaction_'.date('dmHis'); @endphp" data-pageO="p"
                ...
                <tbody>
                    @forelse($resp as $i => $res)
                        <tr style="{{ $i % 2 == 0 ? 'background-color: #f8f9fa' : 'background-color: #fff' }}">
                            ...
                            <td>{{ $res['partybillno'] ?? '' }}</td>
                            <td style="text-align:right">{{ $res['basic'] ?? '' }}</td>
