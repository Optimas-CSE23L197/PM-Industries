...
@section('content')
    <div class="card">
        <div class="card-header py-1">
            <label class="card-title">Raw Item Type</label>
            @include('includes.print_btn')
        </div>
        <div class="card-body print">
            {{-- company header --}}
            <div
                style="text-align: center; margin-bottom: 20px; border-bottom: 2px solid #343a40; padding-bottom: 10px;">
                <h3 style="margin: 0; font-weight: bold; letter-spacing: 1px">
                    {{ $compnm ?? 'PM-Industries' }}
                </h3>
                <h5 style="margin: 10px 0 0 0; text-transform: uppercase; letter-spacing: 2px; color: #343a40;">Raw Item Type Master List</h5>
            </div>
            {{-- company header end --}}

            <table class="table table-bordered" width="100%" border="1"
                data-tableName="@php echo 'Raw-Item-Type-Master_'.date('dmHis'); @endphp" data-pageO="p"
                data-pdfmode="download" cellspacing="0" cellpadding="4">
                <thead>
                    <tr style="background-color: #343a40; color: #fff;">
                        <th style="width: 20%; text-align: center;">Sl. No</th>
                        <th style="width: 55%; text-align: left;">Name</th>
                ...
                <tbody>
                    @forelse($resp as $i => $res)
                        <tr style="{{ $i % 2 == 0 ? 'background-color: #f8f9fa' : 'background-color: #fff' }}">
                            ...
                            <td>{{ $res['name'] ?? '' }}</td>
                            <td style="color: {{ ($res['active_yn'] ?? 'N') === 'Y' ? 'green' : 'red' }}; text-align: center;">
                                {{ ($res['active_yn'] ?? 'N') === 'Y' ? 'Active' : 'Inactive' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
