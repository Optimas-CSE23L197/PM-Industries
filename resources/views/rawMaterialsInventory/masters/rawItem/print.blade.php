    ...
    @section('content')
        <div class="card">
            <div class="card-header py-1">
                <label class="card-title">Raw Item</label>
            ...
            <div class="card-body print">
                ...
                <div
                    ...
                    <h3 style="margin: 0; font-weight: bold; letter-spacing: 1px">
                        {{ $compnm ?? 'TravelPlusCloud' }}</h3>
                    <h5 style="margin: 10px 0 0 0; text-transform: uppercase; letter-spacing: 2px; color: #343a40;">Raw Item
                        Master List</h5>
                </div>
                {{-- company header end --}}

                <table class="table table-bordered" width="100%" border="1"
                    data-tableName="@php echo 'Raw-Item-Master_'.date('dmHis'); @endphp" data-pageO="p"
                    data-pdfmode="download" cellspacing="0" cellpadding="4">
                    <thead>
                        <tr style="background-color: #343a40; color: #fff;">
                            <th style="width: 10%; text-align: center;">No</th>
                            <th style="width: 50%; text-align: left;">Name</th>
                            <th style="width: 25%; text-align: right;">Type</th>
                    ...
                    <tbody>
                        @forelse($resp as $i => $res)
                            <tr style="{{ $i % 2 == 0 ? 'background-color: #f8f9fa' : 'background-color: #fff' }}">
                                ...
                                <td>{{ $res['name'] ?? '' }}</td>
                                <td style="text-align: right">{{ $res['type_name'] ?? '' }}</td>
