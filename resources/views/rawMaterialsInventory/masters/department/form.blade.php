            ...
            <div class="card">
                <div class="card-body">
                    <form id="departmentForm" method="post">
                        ...
                        <input type="hidden" name="code" id="code" value="{{ $deprt['code'] ?? '' }}"/>
                        <input type="hidden" name="active_yn" id="active_yn" value="{{ $deprt['active_yn'] ?? 'Y' }}"/>

                        <div class="form-group row">
                            <label for="name" class="col-md-2 required">Name</label>
                            <div class="col-md-4">
                                <input type="text" name="name" id="name" value="{{ $deprt['name'] ?? '' }}" class="form-control form-control-sm" autofocus required/>
                            </div>
                        </div>
    ...
    @push('js')
        <script>
            ...
            $('#departmentForm').submit(function(e){
                ...
                $.ajax({
                    url : "{{ Route('rawMaterialsInventory.Savedepartment') }}",
                    type: 'post',
                    data: formData,
                    beforeSend:function(){
                    ...
                    success:function(resp){
                        ...
                        if (!resp.error) {
                            mtd.show_msgT(1, "{{Route('rawMaterialsInventory.departmentList')}}", message, 1);
                        } else {
